...
int main(int argc, char* argv[]) {
    ...
    if ((argc != 2) && (argc != 3)) {
        printf("Usage: %s <fileInput> [<fileOutput>]\n", argv[0]); //[<fileOutput>] è opzionale, posso anche non metterlo
        return 1;
    }
...
Student** loadStudentsFromFile(FILE* f, int* outCount) {
    char* line;
    Student** students = NULL;  //non è alloocato (punta a NULL)
    int count = 0;
    ...
    while((line = readLine(f)) != NULL) {
        ...
        count++;
        free(line);  //la faccio perchè la readLine fa la malloc
    }
    students = realloc(students, sizeof(Student*) * (count + 1));
    students[count] = NULL; // Terminatore
    
    *outCount = count; // voglio che restituisce quanti studenti ha fatto e quindi modifico il parametro passato per referenza(*outCount)
    return students;
}

void saveStudentsToFile(FILE* file, Student** students) {
    for(Student** temp = students; *temp; temp++) {   //finisce quando ho NULL e sono arrivato alla fine del vettore
...
void disposeStudentArray(Student** students) {
    ...
    free(students);  //uccido il vettore di studenti
}